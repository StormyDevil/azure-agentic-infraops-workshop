...
| --- | --- | --- |
| **Functional Requirements** | ✅ Complete | 5 features (F1–F5), roles, data model defined |
...
| **Regional Preferences** | ✅ Complete | westeurope (SWA), swedencentral (Storage) |
...
| **Primary Region** | `westeurope` (SWA), `swedencentral` (Storage) |
...
│                                                      │
│  ┌──────────────┐      ┌───────────────────────┐    │
│  │  SPA Frontend │      │  Managed Functions    │    │
│  │  (React/Vue)  │─────▶│  /api/teams           │    │
│  │               │      │  /api/scores          │    │
│  └──────────────┘      │  /api/awards          │    │
│                         └──────────┬────────────┘    │
│                                    │                 │
│  GitHub OAuth (built-in auth)      │                 │
...
3. **GitHub auth built-in** — SWA natively supports GitHub OAuth with role assignments via `staticwebapp.config.json`, eliminating the need for Azure AD B2C or custom auth middleware.
4. **Table Storage for simplicity** — scoring data is simple key-value with known partition keys (team names). No relational joins needed. At <1 GB and <10K transactions/month, cost is effectively $0.
...
| --- | --- | --- |
| **Identity** | GitHub OAuth via SWA built-in auth. Role-based access (facilitator vs participant) via `staticwebapp.config.json`. No custom identity provider needed. | 8/10 |
...
| Azure Storage Account | **Standard LRS** | `swedencentral` | ~$0.05 | Cheapest option; LRS sufficient for event data |
| Azure Table Storage | *(included in Storage)* | `swedencentral` | ~$0.00 | <1 GB data, <10K transactions — effectively free |
| Azure Functions | **Consumption (Managed)** | `westeurope` | $0.00 | Included in SWA Standard; <1M executions |
| Application Insights | **Free tier (5 GB/mo)** | `swedencentral` | $0.00 | Optional but recommended for API observability |
| **Total** | | | **~$9.05** | |

### SKU Alternatives Considered

| Alternative | Cost | Why Not Selected |
| --- | --- | --- |
| SWA Free tier | $0/mo | No custom auth, no SLA, limited storage (250 MB) |
| App Service B1 + Functions | ~$14/mo | More expensive, separate Function App needed, more operational overhead |
| Container Apps Consumption | ~$0-5/mo | More complex deployment, no built-in auth, container packaging overhead |
| Azure SQL Basic | ~$5/mo | Overkill for simple key-value scoring data |
...
| ADR-2 | **Data persistence** | Azure Table Storage (Standard LRS) | Cosmos DB, Azure SQL, Blob Storage | Simplest and cheapest for key-value scoring data; no relational features needed |
| ADR-3 | **Authentication** | GitHub OAuth (SWA built-in) | Azure AD B2C, Auth0, custom JWT | Zero-config, free, participants already have GitHub accounts |
...
| ADR-7 | **Region split** | westeurope (SWA) + swedencentral (Storage) | Single region | SWA limited region availability; Storage in swedencentral for GDPR |
